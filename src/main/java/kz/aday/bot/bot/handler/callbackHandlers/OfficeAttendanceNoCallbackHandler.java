...
public class OfficeAttendanceNoCallbackHandler extends AbstractHandler implements CallbackHandler {
  ...
  public boolean canHandle(CallbackQuery callback) {
    ...
    return CallbackState.ATTENDANCE_NO.name().equals(data[0]);
  }
  ...
  public void handle(CallbackQuery callback, AbsSender sender) throws Exception {
    ...
    if (optionalUser.isPresent()) {
      ...
      officeAttendanceService.save(user.getId(), user.getPreferedName(), user.getCity(), false);
      sendMessage(user, Messages.THANKS_WONT_COME, getMessageId(callback), sender);
    }
  }
}
