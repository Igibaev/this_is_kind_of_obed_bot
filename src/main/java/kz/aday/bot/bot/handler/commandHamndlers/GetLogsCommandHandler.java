...
public class GetLogsCommandHandler extends AbstractHandler implements CommandHandler {
  ...
  public void handle(Update update, AbsSender sender) throws Exception {
    ...
    try {
      ...
      if (logFileToSend != null
          && logFileToSend.exists()
          && logFileToSend.isFile()
          && logFileToSend.length() > 0) {
        sendLogsFile(user, logFileToSend, linesCount, getMessageId(update), sender);
        log.info(
            "Файл логов {} ({} строк) успешно отправлен в чат {}.",
            logFileToSend.getName(),
            linesCount,
            user.getId());
      } else {
        sendMessage(
  ...
  /**
   * Парсит количество строк из команды /getlogs. Например, для "/getlogs 100" вернет 100. Для
   * "/getlogs" вернет дефолтное значение.
   */
  ...
  private void sendLogsFile(
      User user, File file, int linesCount, Integer lastUserSendedMessageId, AbsSender sender) {
    SendDocument sendDocument = new SendDocument();
    sendDocument.setChatId(user.getChatId());
    sendDocument.setDocument(new InputFile(file));
    sendDocument.setCaption(
        String.format("Последние %d строк логов. Имя файла: %s", linesCount, file.getName()));
    ...
    try {
      ...
      log.error(
          "Ошибка при отправке файла логов {} в чат {}: {}",
          file.getName(),
          user.getId(),
          e.getMessage(),
          e);
