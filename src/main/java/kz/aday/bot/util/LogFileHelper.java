...
public class LogFileHelper {
  ...
  private static final String LOG_FILE_PREFIX =
      "telegram-bot"; // Префикс имени лог-файла из logback.xml
  ...
  /**
   * Находит самый свежий лог-файл (включая .gz архивы) и извлекает из него указанное количество
   * последних строк. Создает временный файл с этими строками.
   *
   * @param linesCount Количество последних строк для извлечения. Если <= 0, используется
   *     DEFAULT_LINES_COUNT.
   * @return Объект File с последними строками лога, или null, если файл не найден или произошла
   *     ошибка.
  ...
  public static File getLatestLogFileWithLastLines(int linesCount) throws IOException {
    ...
    Optional<Path> latestLogPath =
        Files.list(logsDir)
            .filter(
                p ->
                    p.getFileName().toString().startsWith(LOG_FILE_PREFIX)
                ...
                Comparator.comparingLong(
                    p -> {
                      try {
                        ...
                        logger.error(
                            "Не удалось получить время модификации файла {}: {}",
                            p,
                            e.getMessage());
                        return 0L;
                      }
    ...
    Deque<String> lastLines =
        new ArrayDeque<>(actualLinesCount); // Двусторонняя очередь для хранения последних строк
    ...
    try (OutputStream os = new FileOutputStream(tempFile)) {
      ...
      logger.info(
          "Временный файл логов создан: {} ({} строк)",
          tempFile.getAbsolutePath(),
          lastLines.size());
      return tempFile;
    } catch (IOException e) {
      logger.error(
          "Ошибка при записи временного файла логов {}: {}",
          tempFile.getAbsolutePath(),
          e.getMessage(),
          e);
