...
class GetAttendanceStatsForMonthStateHandlerTest {
  ...
  private GetAttendanceStatsForMonthStateHandler handler;
  ...
  void setUp() throws Exception {
    ...
    handler = new GetAttendanceStatsForMonthStateHandler();
  }

  @Test
  void canHandle_returnsTrue_whenStateIsGetAttendanceStatsMonth() {
    assertTrue(handler.canHandle(State.GET_ATTENDANCE_STATS_MONTH.getDisplayName()));
  }

  @Test
  void handle_sendsMonthReport_whenAdmin() throws Exception {
    User admin = adminUser(City.ALMATA);
    when(userService.findByIdOptional(CHAT_ID_STRING)).thenReturn(Optional.of(admin));
    when(officeAttendanceService.getCurrentMonthAttendanceStats(City.ALMATA))
        .thenReturn("7 visits");
    ...
    assertTrue(messageCaptor.getValue().getText().contains("7 visits"));
  }
}
