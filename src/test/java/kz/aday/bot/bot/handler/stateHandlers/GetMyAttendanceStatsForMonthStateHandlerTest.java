...
class GetMyAttendanceStatsForMonthStateHandlerTest {
  ...
  private GetMyAttendanceStatsForMonthStateHandler handler;
  ...
  void setUp() throws Exception {
    ...
    handler = new GetMyAttendanceStatsForMonthStateHandler();
  }

  @Test
  void canHandle_returnsTrue_whenStateIsGetMyAttendanceStatsMonth() {
    assertTrue(handler.canHandle(State.GET_MY_ATTENDANCE_STATS_MONTH.getDisplayName()));
  }

  @Test
  void handle_sendsPersonalMonthReport() throws Exception {
    User user = readyUser(City.ALMATA);
    when(userService.findByIdOptional(CHAT_ID_STRING)).thenReturn(Optional.of(user));
    when(officeAttendanceService.getCurrentMonthAttendanceStatsForUser(City.ALMATA, CHAT_ID_STRING))
        .thenReturn("1 visit");
    ...
    assertTrue(messageCaptor.getValue().getText().contains("1 visit"));
  }
}
